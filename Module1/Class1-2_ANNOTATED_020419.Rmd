...
```{r}
barplot(table(nmes.data$female, nmes.data$eversmk), beside = TRUE, legend.text=TRUE, main="Counts individuals who smoked by sex")
```
...
prop.table(table(nmes.data$eversmk, nmes.data$lc5), margin=1)
...
nmes.data %>% group_by(female, eversmk, lc5) %>% summarise(n=n()) %>% mutate(frac = n/sum(n)) %>% filter(lc5 == "LC") %>%
  ggplot(aes(x=eversmk, y=frac, fill=female)) + 
  geom_bar(stat="identity", position="dodge")
  
nmes.data %>% group_by(female, eversmk, lc5) %>% summarise(n=n()) %>% mutate(frac = n/sum(n)) %>% filter(lc5 == "LC") %>%
  ggplot(aes(x=female, y=frac, fill=eversmk)) + 
  geom_bar(stat="identity", position="dodge") + 
  ylab("Rate of lung cancer") + xlab("Sex")
...
```{r}
nmes.data<-nmes.data %>% mutate(disease=factor(lc5 == "LC" | chd5 == "CHD", levels=c(FALSE, TRUE), labels=c("No disease", "Yes disease")))

table(nmes.data$disease, nmes.data$lc5)
table(nmes.data$disease, nmes.data$chd5)
```
