...
ggplot(aes(date, death), data=ny.data) + 
  geom_point()+
  geom_line(data=df,aes(x=date, y=pred.model1, colour = "blue"), size=2) + 
...
To get you started, I have written a skeleton of the function, with some spots for you to fill in. If you don't want to do things this way, that's OK too. Note that this chunk of code will not currently be evaluated -- you can change that once you have a working function.
...
getEsts<-function(cityName){
  ...
  #modOut<-glm(<MODEL YOU WANT TO FIT>, data=currDat, family=poisson)
  modOut<-glm(death~pm10, data=currDat, family=poisson)
