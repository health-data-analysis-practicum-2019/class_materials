...
ggplot(aes(date, death), data=ny.data) + 
  geom_point()+
  geom_line(data=df,aes(x=date, y=pred.model1, colour = "blue"), size=2, alpha=0.8) + 
  geom_line(data=df,aes(x=date, y=pred.model2, colour="red"), size=0.5) + 
...
getEsts<-function(cityName){
  ...
  ## this will return the estimate and its standard error and it's CI as a vector of length 4
  ## using a list allows you to return multiple values from your function
...
chiOut<-getEsts("chicago")$toReturn
